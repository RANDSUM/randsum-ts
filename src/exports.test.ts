...
import d  from './d';
import { D4, D6, D8, D10, D12, D20, D100 } from './dConstants';
import { RollAccessor, RollParameters, RollModifier } from './types';
import rollLog from './rollLog';

describe('Index Exports', () => {
  test('include d as default', () => {
    expect(ex.default).toEqual(d);
  });
  test('include d as named export', () => {
    expect(ex.d).toEqual(d);
  });
  test('includes rollLog', () => {
    expect(ex.rollLog).toEqual(rollLog);
  })
  test('include premade Dice Constants', () => {
